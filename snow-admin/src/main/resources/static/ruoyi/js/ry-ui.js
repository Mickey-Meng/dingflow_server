...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            startFlowReq: function (id,fromInfoId) {
                var startFlowReqReturn = null;
                console.log(fromInfoId);
                console.log("fromInfoId");
                $.ajax({
                    type: "post",
            ...
            startFlowPage: function (id, width, height) {

                console.log("aaaaaaaa" + id)
                let startFlowReqReturn = $.operate.startFlowReq(id);

                console.log(startFlowReqReturn)
                console.log(startFlowReqReturn.code)
                console.log(startFlowReqReturn.code === 'REDIRECT')

                if (startFlowReqReturn.code != 'REDIRECT') {

                    $.modal.alertWarning("启动成功");
                    return;
                }
                console.log("bbbbbbb" + id)
                url = startFlowReqReturn.data;
                var title = '设计器';
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
