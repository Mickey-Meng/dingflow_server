...
public class FlowableTaskServiceImpl implements FlowableTaskService {
    ...
    public PageModel<TaskVO> findTasksByUserId(String userId, TaskBaseDTO taskBaseDTO) {
        ...
        TaskQuery taskQuery = taskService.createTaskQuery()
                .or()
                .taskCandidateOrAssigned(userId).taskCandidateOrAssigned("$INITIATOR");
        ...
        List<TaskVO> taskVoList = taskList.stream().map(t -> {
            ...
            if ("$INITIATOR".equals(taskVO.getAssignee())){
    ...
    @Override
    public String submitTaskFrom(String taskId, Long userId, Integer version, HashMap hashMap) {
        Task task = this.getTask(taskId);

        if (StringUtils.isEmpty(task)) {
            log.info("完成任务时，该任务ID:%不存在", taskId);
            throw new BusinessException(String.format("该任务ID:%不存在", taskId));
        }
        ...
        taskService.setVariable(task.getId(), "history" + task.getId() + "historyFromdatilURL", "/toFormRecordDetail?id=" + version);

        Object isPass = hashMap.get("isPass");
        if (isPass!=null) {

            taskService.setVariable(task.getId(), "isPass" , isPass);
            taskService.setVariable(task.getId(), task.getId()+"isPass" , isPass);
        }

        Map<String, Object> param = new HashMap<>();

        Map<String, Object> paramMap = BeanUtil.beanToMap(new FinishTaskDTO());
//        taskService.claim(task.getId(), userId.toString());
        taskService.complete(task.getId(), param);
        taskSkipService.autoSkip(task.getProcessInstanceId());
        String processDefinitionId = task.getProcessDefinitionId();
        return processDefinitionId;
//        Object qwe = taskService.getVariable(task.getId(), "qwe");
